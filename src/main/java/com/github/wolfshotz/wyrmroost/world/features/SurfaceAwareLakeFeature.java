...
public class SurfaceAwareLakeFeature extends Feature<BlockStateConfiguration>
{
    ...
    public boolean place(WorldGenLevel level, ChunkGenerator generator, Random random, BlockPos pos, BlockStateConfiguration config)
    {
        ...
        {
            pos = pos.below(4);
            if (level.startsForFeature(SectionPos.of(pos), StructureFeature.VILLAGE).findAny().isPresent()) return false;
            else
            {
                    ...
                    for (int j3 = 0; j3 < 16; ++j3)
                    {
                        for (int j4 = 4; j4 < 8; ++j4)
                        {
                            if (complex[(i2 * 16 + j3) * 8 + j4])
                            {
                                ...
                                if (isDirt(level.getBlockState(blockpos).getBlock()) && level.getBrightness(LightLayer.SKY, pos.offset(i2, j4, j3)) > 0)
                                {
